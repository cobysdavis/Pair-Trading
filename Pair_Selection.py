...
class Pair_Selector:
    ...
    def MeanReversionSpeed(self,data):
        x=data[0:len(data)-1]
        y=data[1:len(data)]
        x=np.array(x)
        y=np.array(y)
    ...
    def selectStats(self):
        ...
        urlUSA = 'https://raw.githubusercontent.com/cobysdavis/Pair-Trading/Pulkit/USA.csv'
        data = pd.read_csv(urlUSA, error_bad_lines=False)
        #load data for stocks and statistics
        urlStats = 'https://raw.githubusercontent.com/cobysdavis/Pair-Trading/Pulkit/Statistics.csv'
        statistics=pd.read_csv(urlStats,error_bad_lines=False)
        ...
        #Filter based on Beta and Market Cap
        pairsFilter1={}
            ...
            for pairs in pairsFilter1.keys():
                ...
                for stock in (bins):
                    ...
                    if (any([math.isnan(x) for x in s1])) or (any([math.isnan(x) for x in s2])):
                        ...
                        ms= self.MeanReversionSpeed(spread)
